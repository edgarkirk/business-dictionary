...
class DictionaryController {
    ...
    public TermResponse create(@Valid @RequestBody CreateTermRequest request) {
        return toResponse(dictionaryService.create(request.term(), request.definition()));
    }

    @GetMapping("/terms/{term}")
    @Operation(summary = "Get a business term by name")
    public TermResponse findByTerm(@PathVariable String term) {
        return toResponse(dictionaryService.findByTerm(term));
    }

    @PutMapping("/terms/{term}")
    @Operation(summary = "Update the definition of an existing business term")
    public TermResponse updateDefinition(
