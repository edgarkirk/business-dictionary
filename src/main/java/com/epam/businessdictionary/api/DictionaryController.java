...
class DictionaryController {
    ...
    TermResponse createTerm(@Valid @RequestBody CreateTermRequest request) {
        return TermResponse.from(dictionaryService.createTerm(request.term(), request.definition()));
    }

    @GetMapping("/terms/{term}")
    @Operation(summary = "Retrieve a business term by name")
    TermResponse getTerm(@PathVariable String term) {
        return TermResponse.from(dictionaryService.getTerm(term));
    }

    @PutMapping("/terms/{term}")
    @Operation(summary = "Update the definition of an existing term")
    TermResponse updateTerm(@PathVariable String term,
                            @Valid @RequestBody UpdateTermRequest request) {
        return TermResponse.from(dictionaryService.updateTerm(term, request.definition()));
    }
}
