...
@Tag(name = "Business Dictionary", description = "Manage business terms and definitions")
class DictionaryController {
    ...
    public TermResponse createTerm(@Valid @RequestBody CreateTermRequest request) {
        return toResponse(dictionaryService.createTerm(request.term(), request.definition()));
    }
    ...
    public TermResponse getTerm(@PathVariable String term) {
        return toResponse(dictionaryService.getTerm(term));
    }

    @PutMapping("/terms/{term}")
    @Operation(summary = "Update the definition of an existing term")
    public TermResponse updateTerm(@PathVariable String term, @Valid @RequestBody UpdateTermRequest request) {
        return toResponse(dictionaryService.updateTerm(term, request.definition()));
    }
